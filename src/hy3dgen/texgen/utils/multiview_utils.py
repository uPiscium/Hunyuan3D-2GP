...
class Multiview_Diffusion_Net:
    def __init__(self, config) -> None:
        ...
        custom_pipeline_path = os.path.join(
            os.path.dirname(current_file_path), "..", "hunyuanpaint"
        )
        ...
        self.pipeline = pipeline.to(
            self.device
        )  # only for cosmetics and not display the warning
    ...
    def __call__(self, input_image, control_images, camera_info):
        ...
        for i in range(len(control_images)):
            control_images[i] = control_images[i].resize(
                (self.view_size, self.view_size)
            )
            if control_images[i].mode == "L":
                control_images[i] = control_images[i].point(
