...
class Hunyuan3DPaintPipeline:
    ...
    def from_pretrained(cls, model_path):
        ...
        if os.path.exists(model_path):
            ...
            if os.path.exists(delight_model_path) and os.path.exists(
                multiview_model_path
            ):
                return cls(
                    Hunyuan3DTexGenConfig(delight_model_path, multiview_model_path)
                )
        ...
        if not os.path.exists(model_path):
            ...
            if not os.path.exists(delight_model_path) or not os.path.exists(
                multiview_model_path
            ):
                try:
                    ...
                    delight_model_path = os.path.join(
                        model_path, "hunyuan3d-delight-v2-0"
                    )
                    multiview_model_path = os.path.join(
                        model_path, "hunyuan3d-paint-v2-0"
                    )
                    return cls(
                        Hunyuan3DTexGenConfig(delight_model_path, multiview_model_path)
                    )
        ...
        raise FileNotFoundError(
            f"Model path {original_model_path} not found and we could not find it at huggingface"
        )
    ...
    def bake_from_multiview(
        ...
        for view, camera_elev, camera_azim, weight in zip(
            ...
            project_texture, project_cos_map, project_boundary_map = (
                self.render.back_project(view, camera_elev, camera_azim)
            )
